...
export async function probeUnifiedControls(page, label) {
 return page.evaluate(label => {
  ...
  for(const element of controls){
   ...
   if(!element.matches(':disabled,[aria-disabled="true"]')&&element.matches('.danger-button,.danger-active,.ptw-control-button.is-emergency-kill,.ptw-notice.is-block,.ptw-notice.is-kill,.ptw-metric-card.is-block,.ptw-metric-card.is-kill')) checks.dangerBackground=bg.join(',')===rgba(getComputedStyle(document.documentElement).getPropertyValue('--ts-control-danger-bg')).join(',');
